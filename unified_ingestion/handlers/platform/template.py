...
class MyPlatformArtifactHandler(PlatformArtifactHandler):
    ...
    async def handle(
        ...
        if "artifact1" in filename:
            ...
            return HandlerResult(success=False, chunks=[], error=f"Unknown MyPlatform artifact: {path}")
...
class MyArtifact1Handler(Handler):
    ...
    async def handle(
        ...
        """Parse MyPlatform artifact type 1."""
        import json
        import logging
        from unified_ingestion.handlers.base import Chunk
...
class MyArtifact2Handler(Handler):
    ...
    async def handle(
        ...
        """Parse MyPlatform artifact type 2."""
        import logging
        ...
        try:
            text = content.decode("utf-8", errors="ignore")
            # TODO: Implement parsing logic for artifact type 2
            return HandlerResult(
                success=True,
                chunks=[],
                metadata={
