...
# NOTE: Observability tools currently return raw metric data without
# formatting or aggregation. For production use, consider adding:
# - Metric aggregation (avg, p50, p95, p99)
# - Time-series formatting for dashboard consumption
# - Alert threshold formatting and summarization
...
class MetricsCollectorTool(PDLCBaseTool):
    ...
    async def _llm_execute(self, input: ToolInput) -> ToolOutput:
        ...
        return ToolOutput(
            result={"metrics": metrics}, metadata={"collected": True, "method": "llm"}
        )
    ...
    async def _fallback(self, input: ToolInput) -> ToolOutput:
        ...
        return ToolOutput(
            result={"metrics": metrics}, metadata={"collected": True, "method": "fallback"}
        )

    async def _collect_metrics_llm(
        self, source: str, query: str, time_range: str
    ) -> List[Dict[str, Any]]:
        try:
...
class LogAggregatorTool(BaseTool):
    ...
    async def execute(self, input: ToolInput) -> ToolOutput:
        ...
        try:
            ...
            return ToolOutput(result={"logs": logs}, metadata={"aggregated": True, "method": "llm"})
        except Exception as e:
            logger.warning("LLM log aggregation failed: %s", e)
            return ToolOutput(
                result=None,
                error="LLM unavailable — log aggregation requires an LLM provider",
                metadata={},
            )

    async def _aggregate_logs_llm(
        self, sources: List[str], query: str, time_range: str, level: str
    ) -> List[Dict[str, Any]]:
        try:
            from llm.router import get_router
...
class AlertManagerTool(PDLCBaseTool):
    ...
    async def _fallback(self, input: ToolInput) -> ToolOutput:
        ...
        return ToolOutput(result=result, metadata={"managed": True, "method": "fallback"})

    async def _manage_alert_llm(self, alert: Dict[str, Any], action: str) -> Dict[str, Any]:
        try:
            from llm.router import get_router
...
Generate production-quality alert rules."""
    ...
    async def _manage_alert_fallback(self, alert: Dict[str, Any], action: str) -> Dict[str, Any]:
        return {
            "action": action,
            "success": False,
            "error": "LLM alert management unavailable",
            "available": False,
        }
...
class DashboardGeneratorTool(PDLCBaseTool):
    ...
    async def _llm_execute(self, input: ToolInput) -> ToolOutput:
        ...
        return ToolOutput(
            result={"dashboard": dashboard}, metadata={"generated": True, "method": "llm"}
        )
    ...
    async def _fallback(self, input: ToolInput) -> ToolOutput:
        ...
        return ToolOutput(
            result={"dashboard": dashboard}, metadata={"generated": True, "method": "fallback"}
        )

    async def _generate_dashboard_llm(
        self, metrics: List[str], format: str, title: str
    ) -> Dict[str, Any]:
        try:
            from llm.router import get_router
...
class TracingCollectorTool(BaseTool):
    ...
    async def execute(self, input: ToolInput) -> ToolOutput:
        ...
        try:
            ...
            return ToolOutput(
                result={"traces": traces}, metadata={"collected": True, "method": "llm"}
            )
        except Exception as e:
            logger.warning("LLM trace collection failed: %s", e)
            return ToolOutput(
                result=None,
                error="LLM unavailable — trace collection requires an LLM provider",
                metadata={},
            )

    async def _collect_traces_llm(
        self, trace_id: str, service: str, time_range: str
    ) -> List[Dict[str, Any]]:
        try:
            from llm.router import get_router
...
class SLOTrackerTool(BaseTool):
    ...
    async def execute(self, input: ToolInput) -> ToolOutput:
        ...
        try:
            ...
            return ToolOutput(
                result={"tracking": tracking}, metadata={"tracked": True, "method": "llm"}
            )
        except Exception as e:
...
class AnomalyDetectorTool(BaseTool):
    ...
    async def execute(self, input: ToolInput) -> ToolOutput:
        ...
        try:
            ...
            return ToolOutput(
                result={"anomalies": anomalies}, metadata={"detected": True, "method": "llm"}
            )
        except Exception as e:
    ...
    async def _detect_anomalies_llm(
        self, metrics: List[Dict[str, Any]], sensitivity: str
    ) -> List[Dict[str, Any]]:
        try:
            from llm.router import get_router
