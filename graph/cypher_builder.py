...
class CypherBuilder:
    ...
    @classmethod
    def validate_int_param(cls, value: int, min_val: int = 0, max_val: int = 1000) -> int:
        ...
        try:
            ...
            raise CypherInjectionError(f"Invalid integer value '{value}': {e}") from e

        if int_value < min_val or int_value > max_val:
            raise CypherInjectionError(f"Integer {int_value} out of bounds [{min_val}, {max_val}]")
    ...
    def _validate_rel_type_in_allowlist(self, rel_type: str) -> str:
        ...
        if rel_type not in self._allowed_relations:
            raise CypherInjectionError(
                f"Relationship type '{rel_type}' not in allowed types: {self._allowed_relations}"
            )
        return rel_type

    def match_node(self, labels: list[str], properties: dict[str, Any]) -> "CypherBuilder":
        ...
        # Build label part
        label_str = (
            ":" + ":".join([f"`{lb}`" for lb in validated_labels]) if validated_labels else ""
        )
    ...
    def create_node(self, labels: list[str], properties: dict[str, Any]) -> "CypherBuilder":
    ...
    def create_relationship(
        ...
        from_label_str = ":" + ":".join([f"`{lb}`" for lb in from_labels]) if from_labels else ""
        to_label_str = ":" + ":".join([f"`{lb}`" for lb in to_labels]) if to_labels else ""
    ...
    def match_relationship(
        self, rel_type: str, min_depth: int = 1, max_depth: int = 3, properties: dict | None = None
    ) -> "CypherBuilder":
    ...
    def where_clause(self, prop: str, operator: str, value: Any) -> "CypherBuilder":
        ...
        if op_upper not in self._allowed_operators:
            raise CypherInjectionError(
                f"Invalid operator '{operator}'. Must be one of: {self._allowed_operators}"
            )
    ...
    def add_union(self, other_query: str) -> "CypherBuilder":
        """Add a UNION clause with another query.

        Combines the results of the current query with another Cypher query.
        Both queries must return the same columns.

        Args:
            other_query: The other Cypher query to union with.

        Returns:
            Self for method chaining.
        """
        current = "\n".join(self._parts)
        self._parts = [f"{current}\nUNION\n{other_query}"]
        return self
